...
| `DEVSPACE_AUTH` | `oauth` (default) or `off`. `off` disables the approval page and bearer check. Only use on loopback with no public tunnel. |
...
| `DEVSPACE_OAUTH_ALLOWED_REDIRECT_HOSTS` | `chatgpt.com,localhost,127.0.0.1,www.cursor.com,cursor.com,anysphere.cursor-mcp,mcp` |
